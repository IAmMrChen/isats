...
@section('content')
  ...
  <div v-show="current_type == 'English' && mathjax_load_complete" unselectable="on" style="-moz-user-select:none;" onselectstart="return false;">
    <div v-for="(item, index) in question_data.English" v-show="current_question == item.order">
      <div class="content-padding-50">
        ...
        <div v-for="(temp, cat_index) in item.cat" :class="{'content-read-subject': item.subject_cat, 'content-subject': !item.subject_cat}" unselectable="on" style="-moz-user-select:none;" onselectstart="return false;">
          <span class="txt _timu subject-title" v-html="temp.subject" unselectable="on" style="-moz-user-select:none;" onselectstart="return false;"></span>
          ...
          <el-radio-group v-show="temp.subject_type == 1" v-model="temp.radio_checked" :class="{'content-read-subject-detail': item.subject_cat}">
            <el-radio v-for="(subject_detail, subject_detail_index) in temp.subject_detail" :label="subject_detail.detail_id" class="width-100-percent font-size-18" :class="{'last-answer': (cat_index+1 == item.cat.length && subject_detail_index+1 == temp.subject_detail.length), 'before-last-answer': subject_detail_index+1 < temp.subject_detail.length}">
              <span v-html="subject_detail.detail_content" class="_select_item font-size-16" style="color: black;"></span>
            </el-radio>
          </el-radio-group>
        </div>

        <p class="button" v-if="index != 0" style="left: 50px;" v-on:click="prev_question(index, question_data.Mathematics, 'English')" unselectable="on" style="-moz-user-select:none;" onselectstart="return false;"><a>Prev</a></p>
        <p v-if="!show_submit" class="button" v-on:click="next_question(index, question_data.English, 'English')" unselectable="on" style="-moz-user-select:none;" onselectstart="return false;"><a>Next</a></p>
  ...
  <div v-show="current_type == 'Mathematics' && mathjax_load_complete" unselectable="on" style="-moz-user-select:none;" onselectstart="return false;">
    <div v-for="(item, index) in question_data.Mathematics" v-show="current_question == item.order">
      <div class="content-padding-50">
        <span class="txt font-size-20" v-show="item.subject_cat" v-html="item.subject_cat"></span>
        <div v-for="(temp, cat_index) in item.cat" :class="{'content-read-subject': item.subject_cat, 'content-subject': !item.subject_cat}" unselectable="on" style="-moz-user-select:none;" onselectstart="return false;">
          <span class="txt _timu subject-title" style="" v-html="temp.subject" unselectable="on" style="-moz-user-select:none;" onselectstart="return false;"></span>
          <textarea class="last-answer subject-fill-in-answer" v-model="temp.answer_content" v-show="temp.subject_type == 2"></textarea>
          <!-- <div v-show="temp.subject_type == 1" v-for="(subject_detail, subject_detail_index) in temp.subject_detail" :class="{'last-answer': (cat_index+1 == item.cat.length && subject_detail_index+1 == temp.subject_detail.length), 'before-last-answer': subject_detail_index+1 < temp.subject_detail.length}" class="sel">
            <input class="input" type="radio" v-model="temp.radio_checked" :value="subject_detail.detail_id" />
            <span class="seltxt" v-html="subject_detail.detail_content"></span>
      ...
      <p class="button" v-if="index != 0" style="left: 50px;" v-on:click="prev_question(index, question_data.Mathematics, 'Mathematics')" unselectable="on" style="-moz-user-select:none;" onselectstart="return false;"><a>Prev</a></p>
      <p v-if="!show_submit" class="button" v-on:click="next_question(index, question_data.Mathematics, 'Mathematics')" unselectable="on" style="-moz-user-select:none;" onselectstart="return false;"><a>Next</a></p>
      <p v-if="show_submit" class="button" v-on:click="submit(1)"><a>Submit</a></p>
      <div class="clear"></div>
    </div>
  </div>

  <div v-show="current_type == 'Logic' && mathjax_load_complete" unselectable="on" style="-moz-user-select:none;" onselectstart="return false;">
    <div v-for="(item, index) in question_data.Logic" v-show="current_question == item.order">
      <div class="content-padding-50">
        ...
        <p class="button" v-if="index != 0" style="left: 50px;" v-on:click="prev_question(index, question_data.Mathematics, 'Logic')" unselectable="on" style="-moz-user-select:none;" onselectstart="return false;"><a>Prev</a></p>
        <p v-show="!show_submit" class="button" v-on:click="next_question(index, question_data.Logic, 'Logic')" unselectable="on" style="-moz-user-select:none;" onselectstart="return false;"><a>Next</a></p>
        <p v-if="show_submit" class="button" v-on:click="submit(1)"><a>Submit</a></p>
        <div class="clear"></div>
      </div>
    </div>
      ...
      vm = new vue({
        ...
        methods: {
          getQuestionDatas: function () {
            ...
            $.post('/examination/getexaminationsubjects', params, function (data) {
              if (data.status_code == 200) {
                ...
                setTimeout(function () {
                  dynamicLoadJs('https://cdnjs.cloudflare.com/ajax/libs/mathjax/2.7.5/MathJax.js?config=TeX-MML-AM_CHTML',function(){
                    setTimeout(function () {
                      value_temp.mathjax_load_complete = true;
                    }, 300);
                  });
                }, 300);
              } else {
          ...
          submit: function (origin) {
            ...
            if (origin == 1 && not_do_question.length > 0) {
              ...
              ELEMENT.MessageBox.confirm("You still have questions that are not completed(" + string + "), are you sure to submit?", 'Prompt', {
                confirmButtonText: 'Yes',
                cancelButtonText: 'No',
                type: 'warning'
              }).then(() => {
                obj.submit_method(result);
              }).catch(() => {
                return;        
              });
            } 
          },
          submit_method: function (result) {
          ...
          remindMessage: function () {
            var obj = this;
            ELEMENT.Message.warning({
              message: 'time up, forced submission',
              duration: 2000,
              onClose: function () {
                // obj.submit();
              }
